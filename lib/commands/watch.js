...
function watch() {
    ...
    process.on('SIGINT', () => {
        console.log();
        console.log(tips_1.default.checkDirBeforeExit);
        const [oldDir, newDir] = dirList;
        const { same } = (0, diffDirectories_1.default)(oldDir, newDir);
        if (same) {
            console.log(tips_1.default.dirSameExit);
        }
        else {
            console.log(tips_1.default.dirNotSameExit);
        }
        process.exit();
    });
}
