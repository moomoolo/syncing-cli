    ...
    return diffBlockList.reduce((prev, { omit, diffList }) => {
        ...
        const content = diffList.reduce((prevContent, { oldLineNum, newLineNum, added, removed, value }) => {
            ...
            if (added) {
                ...
                line += (0, colors_1.bgGreen)(`${padStartWithChalk(newLineNum.toString(), newLineNumLen, colors_1.yellow)} ${value}`);
            }
            else if (removed) {
                let raw = `${padStartWithChalk(oldLineNum.toString(), oldLineNumLen, colors_1.yellow)} `;
                newLineNumLen > 0 && (raw += ''.padStart(newLineNumLen + 1));
                raw += value;
                line += (0, colors_1.bgRed)(raw);
            }
            else {
                line += `${padStartWithChalk(oldLineNum.toString(), oldLineNumLen, colors_1.yellow)} `;
                line += `${padStartWithChalk(oldLineNum.toString(), oldLineNumLen, colors_1.yellow)} ${value}`;
            }
