...
function logError(msg) {
    log(`${new Date().toLocaleString()} |${(0, colors_1.red)('ERROR  ')}| ${msg}`);
}
exports.logError = logError;
function logChange(name, parent, type) {
    let res = `${new Date().toLocaleString()} |${(0, colors_1.blue)('CHANGE ')}| `;
    res += `${type === 'file' ? '📃' : '📂'} ${name} in ${(0, colors_1.blue)(parent)}`;
    log(res);
}
exports.logChange = logChange;
function logAddDir(name, parent) {
    log(`${new Date().toLocaleString()} |${(0, colors_1.green)('ADDDIR ')}| 📂 ${name} in ${(0, colors_1.blue)(parent)}`);
}
exports.logAddDir = logAddDir;
function logAddFile(name, parent) {
    log(`${new Date().toLocaleString()} |${(0, colors_1.green)('ADDFILE')}| 📃 ${name} in ${(0, colors_1.blue)(parent)}`);
}
exports.logAddFile = logAddFile;
function logDeleteDir(name, parent) {
    log(`${new Date().toLocaleString()} |${(0, colors_1.red)('DELDIR ')}| 📂 ${name} in ${(0, colors_1.blue)(parent)}`);
}
exports.logDeleteDir = logDeleteDir;
function logDeleteFile(name, parent) {
    log(`${new Date().toLocaleString()} |${(0, colors_1.red)('DELFILE')}| 📃 ${name} in ${(0, colors_1.blue)(parent)}`);
}
exports.logDeleteFile = logDeleteFile;
function logSync(name, parent, type) {
    let res = `${new Date().toLocaleString()} |${(0, colors_1.yellow)('SYNC   ')}| `;
