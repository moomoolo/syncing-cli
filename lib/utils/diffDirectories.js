...
const getAddedList = (res, verbose = false) => {
    return res.diffSet
        ...
        if (type === 'file' && verbose) {
            const newFilePath = path_1.default.join(newDir, relativePath, newName);
...
const getDeletedList = (res, verbose = false) => {
    return res.diffSet
        ...
        if (type === 'file' && verbose) {
            const oldFilePath = path_1.default.join(oldDir, relativePath, oldName);
...
const getChangedList = (res, verbose = false) => {
    return res.diffSet
        ...
        .map((diff) => {
        const { relativePath, name1: oldName, name2: newName, path1: oldDir, path2: newDir, type1: type } = diff;
        const dirDiff = {
            // remove '/' at the beginning
            diffPath: path_1.default.join(relativePath, oldName).substring(1),
            type: type
